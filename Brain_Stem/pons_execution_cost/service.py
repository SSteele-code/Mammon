...
class PonsExecutionCost:
    ...
    def estimate(self, pulse_type: str, frame: Any) -> Dict[str, Any]:
        ...
        try:
            ...
            # Invalid Liquidity Check
            if spread_bps < 0 or frame.environment.atr <= 0:
                # PONS-E-COST-802: INVALID_LIQUIDITY
                logger.info(f"[PONS-E-COST-802] PONS: Invalid liquidity data (spread={spread_bps}, atr={frame.environment.atr}).")
            ...
            if notional <= 0 or avg_volume <= 0:
                ...
                logger.info(f"[PONS-E-COST-801] PONS: Missing trade inputs (notional={notional}, avg_vol={avg_volume}).")
            ...
            if fee_bps is None:
                ...
                if fallback_pct is None:
                    # MNER PONS-E-COST-803
                    logger.info("[PONS-E-COST-803] PONS: Fee schedule missing in standards. Using 30bps ultra-pessimistic fallback.")
                    fee_bps = 30.0
                else:
                    fee_bps = fallback_pct * 10000.0
            
            # 3. TOTAL COST
            # total_cost_bps = slippage + expected_fee
            ...
            # MNER PONS-E-COST-804
            logger.info(f"[PONS-E-COST-804] PONS: Model computation error: {e}")
