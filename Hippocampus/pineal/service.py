...
class Pineal:
    ...
    def secrete_melatonin(self, pulse_type: str = "MINT"):
        ...
        """
        # Hygiene only happens at MINT
        if not enforce_pulse_gate(pulse_type, ["MINT"], "Pineal"):
            return

        logger.info("[PINEAL] Secreting Melatonin... (Multi-Transport Hygiene)")
        try:
            ...
            logger.info("[PINEAL] Melatonin secretion complete.")
        except Exception as e:
            # Standardized MNER for hygiene failure
            logger.info(f"[HIPP-E-P71-705] HYGIENE_CYCLE_FAILED: {e}")
    def _hygiene_redis(self):
        ...
        try:
            ...
            logger.info(f"[HIPP-W-P71-711] Pineal Redis hygiene failed: {e}")
    def _hygiene_duckdb(self):
        ...
        try:
            ...
            for table in tables:
                ...
                # Use standardized librarian gateway
            ...
            logger.info(f"[HIPP-W-P71-712] Pineal DuckDB hygiene failed: {e}")
    def _hygiene_timescale(self):
        ...
        try:
            ...
            logger.info(f"[HIPP-W-P71-713] Pineal TimescaleDB hygiene failed: {e}")
    def finalize_fornix_staging(self, pond, consumed_by_diamond: bool, run_id: str):
        ...
        try:
            ...
            if consumed_by_diamond:
                ...
                logger.info(f"[PINEAL] Fornix staging wiped (Consumed by Diamond: {run_id})")
            else:
                logger.info(f"[PINEAL] Fornix staging PRESERVED (Not consumed by Diamond: {run_id})")
        except Exception as e:
            # Standardized MNER
            logger.info(f"[HIPP-E-P71-706] FORNIX_FINALIZATION_FAILED: {e}")
