...
class DiamondGland:
    ...
    Silver integration. Piece 254: 46-D expansion.
    """
    def __init__(self):
    ...
    def perform_deep_search(self, hours: int = 168):
        ...
        if data.empty or len(data) < 20: 
            logger.info("[DIAMOND] Insufficient training data for Bayesian search. Aborting.")
            return

        # 2. Prepare X (Params) and y (Realized Fitness)
        # Use all 46 PARAM_KEYS
        available_cols = [c for c in PARAM_KEYS if c in data.columns]
        if len(available_cols) < 5:
            logger.info("[DIAMOND] Data schema mismatch. Missing param columns.")
            return
        ...
        logger.info(f"[DIAMOND] Training on {len(X)} tickets across {len(available_cols)} dimensions...")
        ...
        try:
            ...
            # Synthesize Titanium candidate (Best predicted param set)
            ...
            if len(safe_island) == 0:
                logger.warning("[DIAMOND WARNING] No high-fitness island found. Using top 5th percentile.")
                safe_island = X_test[y_mean >= np.percentile(y_mean, 95)]
            
            # Absolute safety guard
            if len(safe_island) == 0:
                logger.critical("[DIAMOND ERROR] Critical: No candidates found for rails. Using full space.")
                safe_island = X_test
            ...
            logger.info("=== [DIAMOND] DEEP SEARCH COMPLETE. TITANIUM + RAILS MINTED. ===\n")
        except Exception as e:
            # PITU-E-P93-906: Bayesian search failure
            logger.info(f"[PITU-E-P93-906] DIAMOND: Bayesian search failed: {e}")
    def _install_titanium(self, params: Dict[str, Any], fitness: float, vault: Dict[str, Any]):
        ...
        logger.info(f"[DIAMOND] Piece 257: Titanium Candidate installed: {param_id}")
