...
class SpreadEngine:
    ...
    def evaluate(self, pulse_type: str, frame: Any) -> Dict[str, Any]:
        ...
        try:
            ...
            # Read bid/ask from ohlcv passthrough
            df = frame.market.ohlcv
            if df.empty or "bid" not in df.columns or "ask" not in df.columns:
                # MNER COUNCIL-E-SPR-702
                logger.info("[COUNCIL-E-SPR-702] SPREAD_ENGINE: Missing bid/ask columns. Using ATR fallback.")
            ...
            if bid <= 0 or ask <= 0 or ask < bid:
                # MNER COUNCIL-E-SPR-701
                logger.info(f"[COUNCIL-E-SPR-701] SPREAD_ENGINE: Invalid quote data (bid={bid}, ask={ask}). Using ATR fallback.")
            ...
            # MNER COUNCIL-E-SPR-704
            logger.info(f"[COUNCIL-E-SPR-704] SPREAD_ENGINE: Unexpected runtime error: {e}")
    ...
    def _calculate_regime(self, spread_bps: float, thresholds: Dict[str, float]) -> str:
        """Categorize the current spread friction."""
